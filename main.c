...
SRWLOCK indexLock = SRWLOCK_INIT;
...
void AddToDelta(wchar_t* name, DWORDLONG ref, wchar_t drive) {
    if (g_deltaCacheCount >= g_deltaCacheCapacity) {
        size_t newCap = (g_deltaCacheCapacity == 0) ? 1000 : g_deltaCacheCapacity * 2;
        void* temp = realloc(g_deltaCache, newCap * sizeof(FileRecord));
        if (!temp) return;
        g_deltaCacheCapacity = newCap;
        g_deltaCache = (FileRecord*)temp;
    }
    
    size_t nameLen = wcslen(name) + 1;
    if (g_deltaArenaUsed + nameLen > g_deltaArenaCapacity) {
        size_t newCap = (g_deltaArenaCapacity == 0) ? 50000 : g_deltaArenaCapacity * 2;
        void* temp = realloc(g_deltaArena, newCap * sizeof(wchar_t));
        if (!temp) return;
        g_deltaArenaCapacity = newCap;
        g_deltaArena = (wchar_t*)temp;
    }
...
void AddToCache(wchar_t* name, DWORDLONG ref, wchar_t drive) {
    if (wcslen(name) < 2) return;

    if (g_cacheCount >= g_cacheCapacity) {
        size_t newCap = (g_cacheCapacity == 0) ? 500000 : g_cacheCapacity * 2;
        void* temp = realloc(g_fileCache, newCap * sizeof(FileRecord));
        if (!temp) return;
        g_cacheCapacity = newCap;
        g_fileCache = (FileRecord*)temp;
    }
    
    size_t nameLen = wcslen(name) + 1; 
    
    if (g_arenaUsedChars + nameLen > g_arenaCapacityChars) {
        size_t newCap = (g_arenaCapacityChars == 0) ? 10000000 : g_arenaCapacityChars * 2;
        void* temp = realloc(g_stringArena, newCap * sizeof(wchar_t));
        if (!temp) return;
        g_arenaCapacityChars = newCap;
        g_stringArena = (wchar_t*)temp;
    }
...
void PathToId(HANDLE hPass, DWORDLONG fileID, wchar_t* outPath, DWORD maxChars) {
    ...
    HANDLE hFile = OpenFileById(hPass, &fileDesc, FILE_READ_ATTRIBUTES, 
...
void SaveAndMapCache() {
    ...
    if (g_fileCache) { free(g_fileCache); g_fileCache = NULL; }
    if (g_stringArena) { free(g_stringArena); g_stringArena = NULL; }
...
__declspec(dllexport) void BuildIndex() {
    AcquireSRWLockExclusive(&indexLock);
    
    CloseIndex();
    
    if (g_deltaCache) { free(g_deltaCache); g_deltaCache = NULL; }
    if (g_deltaArena) { free(g_deltaArena); g_deltaArena = NULL; }
...
__declspec(dllexport) void RunSearch(wchar_t* targetFileName, wchar_t* extFilter, FoundCallback pythonCallback) {
    ...
    if (extFilter) {
        ...
        lowerExt[MAX_PATH - 1] = L'\0';
        _wcslwr(lowerExt);
    }

    HANDLE driveHandles[26] = {0};

    if (g_fileCache && g_stringArena) {
    ...
    ReleaseSRWLockShared(&indexLock);
}
        ...
        if (g_driveUSN[dIdx] != 0) {
            ...
            if (hPass != INVALID_HANDLE_VALUE) {
                ...
                while (DeviceIoControl(hPass, FSCTL_READ_USN_JOURNAL, &readData, sizeof(readData), buffer, sizeof(buffer), &bytesReturned, NULL)) {
                    ...
                    while ((char*)record + record->RecordLength <= buffer + bytesReturned) {
                        if (record->RecordLength == 0) break;
